...
function SupportChatWidget() {
  ...
  const connectWebSocket = (sid) => {
    const wsUrl = `${WS_URL}/ws/support/user/${sid}`;
    ...
    ws.current.onmessage = (event) => {
      ...
      console.log('Support - Received message:', data);
      ...
      } else if (data.Type === 'supporter_joined' || data.Type === 'system') {
        if (data.Message) {
          setMessages(prev => [...prev, data.Message]);
        }
      }
  ...
  const handleSendMessage = () => {
    ...
    const messageData = {
      type: 'message',
      message: newMessage  // Changed from 'content' to 'message' to match backend
    };
  ...
  return (
    <>
      {/* Chat Button - Positioned on LEFT to not overlap with AI assistant */}
      {!isOpen && (
        <Box
          sx={{
            position: 'fixed',
            bottom: 20,
            left: 20, // LEFT side for Support, RIGHT side is for AI
            zIndex: 999 // Lower than AI assistant (1000)
          }}
        >
                ...
                {step === 'form' ? (
                  ...
                  <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <Button
                      ...
                      startIcon={isLoading ? <CircularProgress size={20} /> : <SupportAgentIcon />}
                    >
